 ...
 */
exports.registerDogStatsD = function(host, port) {
...
/*
 * Use case: Timed histograms such as latency.
 * 
 * This will throw if misused.
 *
 * If you don't know what to use for sample_rate, default
 * it to 1 to maintain parity with everything else
 *
 * @param  {String} stat            The stat key
 * @param  {Number} sample_rate     The rate at which to sample this metric.
 *                                  Default value is 1 througout, use that here if in doubt.
 * @param  {Array or Object} tags   Tags to tag the metric with. This can either
 *                                  be an Array (e.g. ['foo', 'bar']) or an
 *                                  Object (e.g. {foo:'bar'}). Optional.
 */
exports.timed_histogram = function(fn) {
    var start = Date.now();
    fn(function(stat, sample_rate, tags) {
        if (arguments.length !== 3) {
            throw new Exception("Please use `timed_histogram` correctly.");
        }

        exports.histogram(stat, Date.now() - start, sample_rate, tags);
    });
}
